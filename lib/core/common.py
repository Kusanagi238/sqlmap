...
from xml.sax import SAXParseException, parse

from extra.beep.beep import beep
from extra.cloak.cloak import decloak
from lib.core.bigarray import BigArray
from lib.core.compat import LooseVersion, cmp, round, xrange
from lib.core.convert import (
    base64pickle,
    base64unpickle,
    decodeBase64,
    decodeHex,
...
from thirdparty.clientform.clientform import ParseError, ParseResponse
...
def getFileItems(
    ...
    try:
        with openFile(filename, "r", errors="ignore") if unicoded else open(
            filename, "r"
        ) as f:
            for line in f:
                if commentPrefix:
...
def checkSums():
    ...
    digest_file = getattr(paths, "DIGEST_FILE", None)
    if digest_file:
        for entry in getFileItems(digest_file):
            match = re.search(r"([0-9a-fA-F]+)\s+([^\s]+)", entry)
            if match:
                ...
                with open(filepath, "rb") as f:
                    content = f.read()
                if not hashlib.sha256(content).hexdigest().lower() == expected.lower():
                    retVal &= False
                    break
